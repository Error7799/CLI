import click
from models import session, Habit, HabitLog, User
...
def add_habit(name, frequency, user_id):
    ...
    if frequency not in ['daily', 'weekly']:
        click.echo("Frequency must be either 'daily' or 'weekly'.")
        return
...
def complete_habit(habit_id):
    ...
    log = HabitLog(habit_id=habit_id, date_completed=date.today()) 
    session.add(log)
    session.commit()
...
def view_logs(habit_id):
    ...
    if logs:
        ...
        consistency = (actual_completions / expected_completions) * 100 if expected_completions else 0
